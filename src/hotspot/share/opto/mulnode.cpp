...
static const Type *lshift_value_helper(const TypeInteger *r1, jlong lo,
...
const Type* LShiftNode::ValueIL(PhaseGVN* phase, BasicType bt) const {
  ...
  if (t1 == Type::BOTTOM || t2 == TypeInt::INT || t2 == Type::BOTTOM) {
  ...
  if (!r1->is_con()) {
    ...
    return bt == T_INT
               ? lshift_value_helper<jint, juint, TypeInt>(r1, lo, hi, shift,
                                                           widen, bt)
               : lshift_value_helper<jlong, julong, TypeLong>(r1, lo, hi, shift,
                                                              widen, bt);
  }
