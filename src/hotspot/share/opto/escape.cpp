  ...
  for (uint i = 1; i < ophi->req(); i++) {
    ...
    if (ptn != nullptr && ptn->scalar_replaceable()) {
      ...
      if (PhaseMacroExpand::can_eliminate_allocation(_igvn, alloc, nullptr)) {
        ...
        if (TraceReduceAllocationMerges) tty->print_cr("%s) %dth input of Phi %d is SR but can't be eliminated.", _compile->method()->name()->as_utf8(), i, ophi->_idx);
        ptn->set_scalar_replaceable(false);
      }
    }
  }

  if (TraceReduceAllocationMerges && !found_sr_allocate) tty->print_cr("%s) Can NOT reduce Phi %d on invocation %d. No SR Allocate as input.", _compile->method()->name()->as_utf8(), ophi->_idx, _invocation);
  ...
  for (DUIterator_Fast imax, i = n->fast_outs(imax); i < imax; i++) {
    ...
    if (use->is_SafePoint()) {
      if (use->is_Call() && use->as_Call()->has_non_debug_use(n)) {
        if (TraceReduceAllocationMerges) tty->print_cr("%s) Can NOT reduce Phi %d on invocation %d. Call has non_debug_use().", _compile->method()->name()->as_utf8(), n->_idx, _invocation);
        return false;
      } else if (has_been_reduced(n->is_Phi() ? n->as_Phi() : n->as_CastPP()->in(1)->as_Phi(), use->as_SafePoint())) {
        if (TraceReduceAllocationMerges) tty->print_cr("%s) Can NOT reduce Phi %d on invocation %d. It has already been reduced.", _compile->method()->name()->as_utf8(), n->_idx, _invocation);
        return false;
      }
    } else if (use->is_AddP()) {
      Node* addp = use;
      for (DUIterator_Fast jmax, j = addp->fast_outs(jmax); j < jmax; j++) {
        ...
        if (!use_use->is_Load() || !use_use->as_Load()->can_split_through_phi_base(_igvn)) {
          if (TraceReduceAllocationMerges) tty->print_cr("%s) Can NOT reduce Phi %d on invocation %d. AddP user isn't a [splittable] Load(). Opcode = %d", _compile->method()->name()->as_utf8(), n->_idx, _invocation, use_use->Opcode());
          return false;
        } else if (load_type->isa_narrowklass() || load_type->isa_klassptr()) {
          if (TraceReduceAllocationMerges) tty->print_cr("%s) Can NOT reduce Phi %d on invocation %d. [Narrow] Klass Load. Opcode = %d", _compile->method()->name()->as_utf8(), n->_idx, _invocation, use_use->Opcode());
          return false;
        }
      }
    } else if (nesting > 0) {
      if (TraceReduceAllocationMerges) tty->print_cr("%s) Can NOT reduce Phi %d on invocation %d. Unsupported user (Opcode %d) at nesting level %d.", _compile->method()->name()->as_utf8(), n->_idx, _invocation, use->Opcode(), nesting);
      return false;
      ...
      if (cast_t == nullptr || cast_t->make_ptr()->isa_instptr() == nullptr) {
        if (TraceReduceAllocationMerges) {
          tty->print_cr("%s) Can NOT reduce Phi %d on invocation %d. CastPP is not to an instance.", _compile->method()->name()->as_utf8(), n->_idx, _invocation);
        }
        return false;
      }
        ...
        if (use->in(0)->is_IfTrue() || use->in(0)->is_IfFalse()) {
          ...
          if (iff->Opcode() == Op_If && iff->in(1)->is_Bool() && iff->in(1)->in(1)->is_Cmp()) {
            ...
            if ((opc == Op_CmpP || opc == Op_CmpN) && !can_reduce_cmp(n, iff_cmp)) {
              if (TraceReduceAllocationMerges) {
                tty->print_cr("%s) Can NOT reduce Phi %d on invocation %d. CastPP %d doesn't have simple control.", _compile->method()->name()->as_utf8(), n->_idx, _invocation, use->_idx);
              }
              return false;
            }
          }
      ...
      if (!can_reduce_cmp(n, use)) {
        if (TraceReduceAllocationMerges) tty->print_cr("%s) Can NOT reduce Phi %d on invocation %d. CmpP/N %d isn't reducible.", _compile->method()->name()->as_utf8(), n->_idx, _invocation, use->_idx);
        return false;
      }
    } else {
      if (TraceReduceAllocationMerges) tty->print_cr("%s) Can NOT reduce Phi %d on invocation %d. One of the uses is %d Opcode %d", _compile->method()->name()->as_utf8(), n->_idx, _invocation, use->_idx, use->Opcode());
      return false;
    }
  }
...
bool ConnectionGraph::can_reduce_phi(PhiNode* ophi) const {
  ...
  if (TraceReduceAllocationMerges) { tty->print_cr("%s) Can reduce Phi %d during invocation %d: ", _compile->method()->name()->as_utf8(), ophi->_idx, _invocation); }
  return true;
}
